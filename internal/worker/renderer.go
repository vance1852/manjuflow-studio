...
func (w *RenderWorker) ProcessOne(ctx context.Context) (bool, error) {
	...
	if renderErr != nil {
		if apperr.IsCode(renderErr, apperr.CodeCanceled) || apperr.IsCode(renderErr, apperr.CodeDeadlineExceeded) {
			return true, renderErr
		}
		permanent, failErr := w.renders.Fail(context.WithoutCancel(ctx), assignment, w.name, renderErr)
		if failErr != nil {
			return true, failErr
		}
