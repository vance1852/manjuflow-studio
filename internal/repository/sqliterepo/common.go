...
// internalise normalises a lookup failure into one repository level error, so
// callers of the storyboard queries see a single failure shape.
func internalise(err error, message string) error {
	if err == nil {
		return nil
	}
	return apperr.Wrap(err, apperr.CodeInternal, "%s", message)
