...
// Complete stores the replayable response for a guarded request.
func (g *Guard) Complete(ctx context.Context, q repository.Querier, claim Claim, status int, response string) error {
...
// Release marks a guarded request as failed so the caller may retry it.
func (g *Guard) Release(ctx context.Context, q repository.Querier, claim Claim) error {
	if !claim.Guarded || claim.Replayed || claim.RecordID == 0 {
		return nil
	}
